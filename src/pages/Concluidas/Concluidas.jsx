...
/**
 * Quando a obra fechou. Vale o carimbo posto pelo store; obra antiga
 * (ou de semente) cai na data prevista e, em ultimo caso, na de criacao
 * — assim nenhuma fica fora do agrupamento.
 */
function quandoFechou(obra) {
  const iso = obra.concluidaEm ?? obra.dataPrevista ?? obra.criadoEm
...
export default function Concluidas() {
  ...
  const { anos, porAno } = useMemo(() => {
    const mapa = {}
    obras.filter(obraConcluida).forEach((obra) => {
  ...
  const total = obras.filter(obraConcluida).length
  ...
  return (
    <AppShell>
      <section className="concluidas">
        ...
        {anos.length === 0 ? (
          <p className="concluidas__vazio">Nenhuma obra concluiu as cinco etapas ainda.</p>
        ) : (
          <>
            {/* ano no topo */}
            <div className="anos">
